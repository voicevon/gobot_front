...
class SitterSystem:
    ...
    # MQTT_TOPIC_CAMERA_RESULT = 'test'   # + sitter_id/camera_id
    ...
    def __init__(self, sitter_id:str) -> None:
    ...
    def save_file_top(self, file_path, payload):
    ...
    def onReceivedMqtt(self, topic, payload):
        ...
        if self.state == self.STATE_TAKING_PICTURE_TOP_CAMERA:
            if topic ==  self.MQTT_TOPIC_CAMERA_RESULT + self.sitter_id + "/top":
                ...
                cv2.waitKey(1)
                cropped_image = self.ToCropped(perspectived_iamge)
    ...
    def ToCropped(self, perspectived_image):
...
sitter_system = SitterSystem("2325")
