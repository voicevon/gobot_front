...
// TODO:  renname to FetchGcode() at consumer class
...
bool GcodeQueue::AppendGcodeCommand(String command){
    ...
    this->SayHello();
    int pre_head = this->_head;
    pre_head++;
    if (pre_head == MESSAGE_COUNT_IN_QUEUE)   
        pre_head =0;

    // // This is solution B:  none blocking mode
    // // Better for slave thread. For example: MQTT received message as message source.
    // if (pre_head == this->tail) 
    //     // Buffer is full
    //     // Serial.println("\n\n\n\n [Warning] GcodeQueue::AppendGcodeCommand()   Buffer is full \n\n\n");
    //     return;

    // // This is solution A:  blocking mode
    // // Better for main thread.
