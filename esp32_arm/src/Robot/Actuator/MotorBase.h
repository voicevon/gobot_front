...
class MotorBase
{
  public:
    ...
    // virtual int initDC( float zero_electric_offset = NOT_SET , Direction sensor_direction = Direction::CW)=0; 
    int initDC( float zero_electric_offset = NOT_SET , Direction sensor_direction = Direction::CW); 
    /**
     ...
     */ 
    // virtual void loopDC()=0;
    void loopDC();
    /**
     * Function executing the control loops set by the controller parameter of the BLMotorBase.
     * 
    ...
    // void move(float target = NOT_SET)=0;
    ...
    /**
     * Function providing BLMotorBase class with the 
    ...
    uint8_t monitor_variables = _MON_TARGET | _MON_VEL | _MON_ANGLE; //!< Bit array holding the map of variables the user wants to monitor
