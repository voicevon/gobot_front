...
float ActuatorStepper_Calculator::Get_Formular_Slope_steps_per_rad(){
    float steps_per_motor_round =  360.0f / this->motor_step_angle_in_degree * this->motor_driver_micro_steps;
    float gear_ratio = (float)this->slave_pulley_teeth_count / (float)this->motor_gear_teeth_count;
    float steps_per_rad =  steps_per_motor_round / TWO_PI * gear_ratio;
    bool debug = false;
    if (debug){
        Logger::Info("ActuatorStepper_PolorConfig::GetStepsPerCncUnit() ");
        Logger::Print("steps_per_motor_round= ", steps_per_motor_round); 
        Logger::Print("  gear ratio= ", gear_ratio);
        Logger::Print("  steps_per_rad= ", steps_per_rad);
    }
    return steps_per_rad;
}


float ActuatorStepper_Calculator::Get_Formular_Slope_steps_per_mm(){
    // The unit is mm here.
    float steps_per_motor_round =  360.0f / this->motor_step_angle_in_degree * this->motor_driver_micro_steps;
