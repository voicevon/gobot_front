...
void CncSolution_FiveBarsBase::_SetCurrentPositionAsHome(EnumAxis homing_axis){
		...
		if (this->_config->IsInverseKinematicHoimg){
			...
			ik_position.alpha = this->_config->GetAxisHomer(AXIS_ALPHA)->GetHomingConfig()->LastHomedPosition;
			ik_position.beta = this->_config->GetAxisHomer(AXIS_BETA)->GetHomingConfig()-> LastHomedPosition;
