...
void CncMover::SpinOnce(){
    ...
    if (gs_MoveBlock_Queue::Instance().BufferIsEmpty()) {
        // Logger::Print("CncMover::SpinOnce() gs_MoveBlock_Queue::  Buffer is Empty", 91);
        return;
    }
    // Logger::Print("CncMover::SpinOnce() gs_MoveBlock_Queue::  Buffer got moveblock", 21);
    ...
    MoveBlock* mb = gs_MoveBlock_Queue::Instance().Withdraw();
