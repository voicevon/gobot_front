...
std::string BoxRobot::GetHomeTrigerStateString(){
...
void BoxRobot::MoveToTargetPosition(){
...
void BoxRobot::RunG1(Gcode* gcode){
...
void BoxRobot::RunG6(Gcode* gcode){
    float pos = gcode->get_value(this->objDcMotor.AxisName);
    // this->singleAxis._actuator->SetTargetAbs(pos);
    if (this->is_absolute_position){
        this->nextPosX.x = pos;
...
void BoxRobot::SpinOnce_BaseExit(){
    this->MoveToTargetPosition();
}

void BoxRobot::Init_Linkage(IrEncoderHelper* sensorHelperBase){
...
// ik_position BoxRobot::ik(float x, float y){
...
IkPositionBase* BoxRobot::IK(FkPositionBase* fk){
    FkPosX* _fk = (FkPosX*)(fk);
    this->objIkPos.alpha = _fk->x;
    return &this->objIkPos;  
}

FkPositionBase* BoxRobot::FK(IkPositionBase* ik){
