...
#ifdef I_AM_GARMENT_BOT_SINGLE_MCU_UNIT_CORE_YZ
...
BotUnitCoreYZ::BotUnitCoreYZ(uint16_t id){
	this->_ID = id;
	// this->objBoxCarrier_hardware = new BoxCarrierHardware();
}

void BotUnitCoreYZ::Init(){
	Serial.print("\n[Info] BotUnitCoreYZ::Init() is entering");
	...
	this->ToState(BotUnitCoreYZ::BOT_STATE::BOT_LOCATING);
	Serial.print("\n[Info] BotUnitCoreYZ::Init() is done.\n");
}

void BotUnitCoreYZ::ExecuteMqttCommand(const char* command){
	// command examples
...
void BotUnitCoreYZ::onDetectedMark(uint16_t BranchNode_id){
   ...
   if (this->objMapNavigator.FetchNode(BranchNode_id, &current_BranchNode)){
	...
	case RoadBranchNode::TASK::LOAD:
		...
		this->ToState(BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_SOURCE);
		break;
	...
	case RoadBranchNode::TASK::SLEEP:
		...
		this->ToState(BotUnitCoreYZ::AGV_PARKED_AT_SOURCE);
		break;
...
void BotUnitCoreYZ::SpinOnce(){
	...
	switch (this->__state){
	case BotUnitCoreYZ::BOT_STATE::BOT_LOCATING:
		//Trying to read RFID.
		// if (this->__rfidReader->PICC_ReadCardSerial() == 123){
		if (this->objRfid.CardId == 123){
			this->ToState(BotUnitCoreYZ::BOT_STATE::BOT_SLEEPING);
		}
		break;
   	case BotUnitCoreYZ::BOT_STATE::BOT_CHARGING:
      	break;
   	case BotUnitCoreYZ::BOT_STATE::BOT_SLEEPING:
      	break;
   	case BotUnitCoreYZ::BOT_STATE::BOT_EMERGENCY_STOPING:
      	break;
   	case BotUnitCoreYZ::BOT_STATE::AGV_MOVING_TO_SOURCE:
		if(this->objAgv.GetState() == AgvBase::AGV_STATE::PARKED)
			this->ToState(BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_SOURCE);
		break;
	case BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_SOURCE:
		this->objBoxCarrier.LoadBox();
		this->ToState(BotUnitCoreYZ::BOT_STATE::ROBOT_LOAD_ALIGN);

		break;
	case BotUnitCoreYZ::BOT_STATE::ROBOT_LOAD_ALIGN:
		if (this->boxCarrierHardware->State == RobotState::IDLE){
			// Last movement is done.
			align_error = this->irSensor->ReadAlignmentError();
			if (align_error < 100){
				this->ToState(BotUnitCoreYZ::BOT_STATE::ROBOT_LOADING);
			}else{
				gcode.concat("Y");
		...
		break;
	case BotUnitCoreYZ::BOT_STATE::ROBOT_LOADING:
		...
		if (this->objBoxCarrier.GetState()== BoxCarrier::BoxMoverState::LOADED)
			this->ToState(BotUnitCoreYZ::BOT_STATE::AGV_MOVING_TO_DESTINATION);
		break;
	case BotUnitCoreYZ::BOT_STATE::AGV_MOVING_TO_DESTINATION:
		if(this->objAgv.GetState() == AgvBase::AGV_STATE::PARKED)
			this->ToState(BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_DESTINATION);
		break;
	case BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_DESTINATION:
		// if(this->objBoxMoverAgent.ReadState() == GarmentBoxMoverAgent::BoxMoverState::UNLOADED)
			// Check battery voltage.
			this->ToState(BotUnitCoreYZ::BOT_STATE::BOT_SLEEPING);
		break;
   default:
		Serial.print("\n [Warning] BotUnitCoreYZ::SpinOnce()  A state with out dealer.! \n\n");
      	break;
   }
}

void BotUnitCoreYZ::ToState(BotUnitCoreYZ::BOT_STATE state){
	String gcode ="G1";

	if (state == this->__state) {
		Serial.print("[Warning] BotUnitCoreYZ::ToState()  Repeated! \n\n");
		return;
	}
 	BotUnitCoreYZ::BOT_STATE new_state = state;
	switch(state){
	case BotUnitCoreYZ::BOT_STATE::BOT_LOCATING:
	...
	case BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_SOURCE:
		...
		// this->obj
		new_state = BotUnitCoreYZ::BOT_STATE::ROBOT_LOAD_ALIGN;
		break;
	case BotUnitCoreYZ::BOT_STATE::ROBOT_LOAD_ALIGN:
		if (this->boxCarrierHardware->State == RobotState::IDLE){
			gcode.concat("Z50");
			this->_gcode_queue->AppendGcodeCommand(gcode);
			new_state = BotUnitCoreYZ::BOT_STATE::ROBOT_LOADING;
		}
		break;
	case BotUnitCoreYZ::BOT_STATE::ROBOT_LOADING:
		if (this->boxCarrierHardware->State == RobotState::IDLE){
			new_state = BotUnitCoreYZ::BOT_STATE::AGV_MOVING_TO_DESTINATION;
		}
		break;
	case BotUnitCoreYZ::BOT_STATE::AGV_MOVING_TO_DESTINATION:
		this->objAgv.ToState(AgvBase::AGV_STATE::FAST_MOVING);
	...
	case BotUnitCoreYZ::BOT_STATE::AGV_PARKED_AT_DESTINATION:
		// this->objBoxMoverAgent.ToPresetState();
		new_state = BotUnitCoreYZ::BOT_STATE::ROBOT_UNLOADING;
		break;

	case BotUnitCoreYZ::BOT_STATE::ROBOT_UNLOADING:
		break;

	default:
		Serial.print("\n\n [Warn]  BotUnitCoreYZ::ToState()   Case Else...");
		break;
   }
   this->__state = new_state;
}

void BotUnitCoreYZ::Test(int test_id){
   	if (test_id == 1) {
		// this->objBoxMoverAgent.PresetState(GarmentBoxMoverAgent::BoxMoverState::LOADING);
		// this->objBoxMoverAgent.ToPresetState();
	   }
...
uint8_t BotUnitCoreYZ::GetMqtt_PubPayload(uint8_t* chars){
	...
	bool debug = false;
	if (debug){
		Serial.print("[Debug] BotUnitCoreYZ::GetMqtt_PubPayload()    length= ");
...
// void BotUnitCoreYZ::onMqttReceived(uint8_t* payload){
