...
void I2c_commu_2023::Init(int min_cell_i2c_address, int cells_count){
    pinMode(2, OUTPUT);
...
bool  I2c_commu_2023::ReadSingleCell(TouchCell_2023* cell){
    ...
    if(index == 0) {
        ...
        if (cell->IsForceOnline){
            ...
            delay(500);
            digitalWrite(2,LOW);
            Serial.println("\n I2c_commu_2023::ReadSingleCell()  No response.  cell_address= " + String(cell->Address));
...
TouchCell_2023* I2c_commu_2023::FindandReadValidateCell(){
    for (uint8_t i= 0; i< this->__CELLS_COUNT; i++){
        TouchCell_2023* pCell = &(this->Cells[i]);
        if (pCell->IsOnline){
