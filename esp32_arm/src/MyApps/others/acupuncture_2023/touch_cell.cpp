...
bool TouchCell_2023::Review_RxBuffer(){
    ...
    for (int i=0; i<14; i++){
        ...
        if (i >=8) byte_index = 1;
        bit_index = i % 8;
        if (__rx_buffer[byte_index] & (1 << bit_index)) {
        ...
        __all_channels[i].Push_to_HistoryValues(__rx_buffer[2 + i]);
