...
void Twh2_XzArmSolution::real_MK_to_FK(MiddleKinematic* mk, FKPosition_XYZRPW* fk){
	...
	// Logger::Debug("Twh2_XzArmSolution::real_MK_to_FK()");
...
void Twh2_XzArmSolution::real_MK_to_IK(MiddleKinematic* mk, IKPosition_abgdekl* ik){
	...
	// Logger::Debug("Twh2_XzArmSolution::real_MK_to_IK()");
	...
	// Logger::Print("Twh2_XzArmSolution::real_MK_to_IK()  alpha", ik->Positions[AXIS_ALPHA]);
	// Logger::Print("Twh2_XzArmSolution::real_MK_to_IK()  beta", ik->Positions[AXIS_BETA]);
}

void Twh2_XzArmSolution::real_IK_to_MK(IKPosition_abgdekl* ik, MiddleKinematic* mk){
	...
	// Logger::Print("Twh2_XzArmSolution::real_IK_to_MK() mk.y", mk->Arm_shaft_At_Y);
...
void Twh2_XzArmSolution::Test_IK_MK_IK(IKPosition_abgdekl* input){
	MiddleKinematic mk;
	IKPosition_abgdekl output;
	// Logger::Info("Twh2_XzArmSolution::Test_IK_MK_IK()");
	real_IK_to_MK(input, &mk);
	real_MK_to_IK(&mk, &output);
}

void Twh2_XzArmSolution::Test_MK_IK_MK(MiddleKinematic* input){
